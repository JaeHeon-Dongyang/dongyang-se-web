...
/**
 * /ask (사내 전용 질문하기)를 사무실 회선에서만 열리게 한다.
 *
 * ASK_ALLOWED_IPS 에 사무실 **공인 IP**를 콤마로 구분해 넣는다 (예: "203.0.113.10,203.0.113.11").
 ...
 */
const allowedIps = (process.env.ASK_ALLOWED_IPS ?? "")
  .split(",")
  .map((ip) => ip.trim())
  .filter(Boolean);
