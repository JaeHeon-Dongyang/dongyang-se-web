...
export const resources: Resource[] = [
  ...
  },
  {
    ...
    summary:
      "워크트리·플랜 모드·리뷰·골처럼 작업 공간을 나누고 긴 작업을 관리하는 세부 기능을 정리했습니다.",
    updatedAt: "2026-09-06",
    body: [
      {
        type: "paragraph",
        text: "지금까지는 어떤 도구를 언제 쓰는지를 다뤘다면, 이번에는 도구 안에서 작업 흐름 자체를 더 낫게 만드는 세부 기능을 다룹니다. 아래 기능은 OpenAI Codex CLI를 기준으로 정리했지만, 상당수는 Claude Code·Antigravity 같은 다른 AI 코딩 에이전트에도 이름만 다르고 비슷한 개념으로 있습니다.",
      },
      {
        type: "link",
      ...
      },
      {
        type: "callout",
        tone: "info",
        text: '도구마다 명칭은 달라도 **"무엇을 위한 기능인지"를 알아두면 다른 AI 코딩 에이전트에서도 그대로 응용할 수 있습니다.**',
      },
      {
        type: "heading",
        text: "작업 공간을 나누는 기능",
      ...
      },
      {
        ...
        rows: [
          [
            "워크트리(Worktree)",
            "지금 작업과 완전히 분리된 복사본 환경을 만들어 그 안에서 실험. 끝나면 병합하거나 통째로 버릴 수 있음",
          ],
          [
            "포크(Fork)",
            "지금까지의 대화 맥락을 그대로 이어받아 독립된 대화로 분기. 같은 문제를 두 가지 방식으로 동시에 시도할 때 사용",
          ],
          [
            "사이드(Side)",
            "메인 대화는 그대로 둔 채 옆에 새 대화창을 열어 별도 질문·검증을 처리. 메인 작업의 맥락을 소모하지 않음",
          ],
        ],
      },
      {
        type: "paragraph",
        text: "워크트리는 브랜치 단위로, 포크는 대화 단위로 독립 공간을 만든다는 점이 다릅니다. 워크트리는 커밋되지 않은 파일(환경 변수, 포트 설정 등)을 자동으로 가져오지 않으므로 새 워크트리를 만들 때마다 직접 채워 넣어야 합니다.",
      },
      {
        type: "heading",
        text: "긴 작업을 관리하는 기능",
      ...
      },
      {
        ...
        rows: [
          [
            "골(Goal)",
            "목표와 완료 조건을 정해두면 몇 턴에서 멈추지 않고 목표를 달성할 때까지 반복 작업. 막혀도 스스로 다시 시도하고, 정말 해결이 안 될 때만 되돌아와 물어봄",
          ],
          [
            "스티어링(Steering)",
            "골처럼 오래 걸리는 작업이 진행되는 도중에 방향을 즉시 수정. 큐에 쌓아뒀다 나중에 반영하는 대신 지금 작업에 바로 끼워 넣음",
          ],
          [
            "플랜 모드(Plan Mode)",
            "작업을 바로 실행하지 않고 계획부터 세우게 한 뒤, 계획을 확인·수정하고 나서 실행하도록 유도",
          ],
          [
            "콤팩트(Compact)",
            "대화가 길어져 맥락(컨텍스트)이 꽉 차기 전에 지금까지 내용을 요약해 압축. 자동으로도 실행되고 필요하면 직접 실행 가능",
          ],
        ],
      },
      {
        type: "paragraph",
        text: "콤팩트는 기억을 요약해서 줄이는 것이라 세부 내용은 흐려질 수 있습니다. 중요한 결정이나 제약 조건은 대화 중간에 따로 메모해두는 편이 안전합니다.",
      },
      {
        type: "heading",
        text: "결과를 확인하는 기능",
      ...
      {
        type: "paragraph",
        text: "작업이 끝났다고 바로 병합하지 않고, 전용 검토 기능으로 변경 사항을 한 번 더 확인합니다.",
      },
      {
        type: "table",
        headers: ["구분", "확인 대상"],
        rows: [
          [
            "베이스 브랜치 비교",
            "지금까지 커밋된 변경 사항 전체를 기준 브랜치와 비교해 검토",
          ],
          ["커밋 전 변경 사항", "아직 커밋하지 않은 지금 작업 내용만 검토"],
        ],
      },
      {
        type: "paragraph",
        text: '검토 결과는 파일 위치와 함께 구체적인 코멘트로 나오므로, 확인 후 고칠 부분만 다시 요청하면 됩니다. 그냥 "리뷰해줘"라고 시키는 것보다 전용 리뷰 기능을 쓰는 편이 더 꼼꼼하게 봅니다.',
      },
      {
        type: "callout",
        tone: "warning",
        text: "리뷰 기능은 실수를 줄여줄 뿐 확인을 대신하지 않습니다. **특히 계산 로직이나 외부에 배포되는 내용은 사람이 최종 확인합니다.**",
      },
      {
        type: "heading",
        text: "대화를 개인화하는 설정",
      ...
      },
      {
        ...
        rows: [
          [
            "메모리(Memory)",
            "이전 대화에서 파악한 내 정보·선호를 기억해 답변에 반영할지 대화별로 켜고 끔",
          ],
          [
            "퍼스널리티(Personality)",
            "답변 말투를 실용적인 기본 톤과 다르게 바꾸는 옵션. 업무용으로는 기본값을 권장",
          ],
          [
            "모델·속도",
            "사용할 모델과 응답 속도(더 빠른 처리에 비용 추가)를 상황에 맞춰 선택",
          ],
          [
            "리스닝(추론 강도)",
            "답을 내기 전 얼마나 깊게 생각을 반복할지 조절. 낮음·보통·높음·최고 단계로 구분",
          ],
          ["스테이터스(Status)", "지금까지 사용량과 처리 속도를 확인"],
        ],
      },
      {
        type: "paragraph",
        text: "메모리는 한 번 대화를 시작한 뒤에는 켜고 끄는 설정을 바꿀 수 없는 도구도 있으므로, 대화를 새로 시작할 때 미리 정해두는 편이 좋습니다.",
      },
      {
        type: "heading",
        text: "필요한 기능을 스스로 찾게 하는 법",
        id: "meta",
      },
      {
        type: "paragraph",
        text: '어떤 프롬프트를 써야 할지 모를 때는 그 질문 자체를 AI에게 맡길 수 있습니다(메타프롬프팅). "이 프로젝트를 디버깅할 수 있는 프롬프트를 만들어줘"처럼 요청하면 AI가 더 구체적인 프롬프트를 만들어주고, 이를 새 세션에 그대로 붙여 넣어 실행하면 됩니다. 골처럼 정형화된 기능을 설정할 때도 메타프롬프팅으로 먼저 프롬프트를 만들면 완료 조건이 더 명확해집니다.',
      },
      {
        type: "paragraph",
        text: "MCP 커맨드로 지금 연결된 외부 도구 목록을 확인할 수 있고, 스킬 찾기 기능으로 다른 사람이 공개한 작업 절차(스킬)를 검색해 가져올 수도 있습니다.",
      },
      {
        type: "callout",
        tone: "warning",
        text: "어떤 기능을 쓰더라도 발주처 원본 자료와 미공개 성과품, 개인정보는 대화나 워크트리에 남기지 않습니다.",
      },
      {
        type: "heading",
        text: "처음 써볼 때 순서",
      ...
      },
      {
        type: "list",
        items: [
          "하나 · 지금 하려는 작업을 메타프롬프팅으로 먼저 다듬습니다.",
          "둘 · 오래 걸리는 작업이면 플랜 모드로 계획부터 세우고 확인합니다.",
          "셋 · 기존 작업과 분리하고 싶으면 워크트리나 포크로 독립된 공간을 만듭니다.",
          "넷 · 반복적으로 오래 진행할 작업은 골로 목표와 완료 조건을 정해 맡깁니다.",
          "다섯 · 진행 중 방향이 바뀌면 스티어링으로 바로 반영합니다.",
          "여섯 · 끝나면 병합 전에 리뷰 기능으로 변경 사항을 확인합니다.",
        ],
      },
      {
        type: "callout",
        tone: "info",
        text: '기능이 많아 보여도 전부 외울 필요는 없습니다. **지금 하는 작업이 "한 번의 질문"인지 "오래 걸리는 목표"인지만 구분해도 어떤 기능을 쓸지 절반은 정해집니다.**',
      },
      {
        type: "video",
        label: "참고 영상 · 코덱스 18가지 필수 기능 12분 정리 (코드팩토리)",
        url: "https://www.youtube.com/watch?v=86B_oGCkyMs",
        description: "이 글에서 정리한 기능들의 원본 소개 영상입니다.",
      },
    ],
