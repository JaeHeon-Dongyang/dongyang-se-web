...
아이콘 버튼)로 고정 (`lib/nav.ts`). 질문하기(`/ask`)는 Gemini Notebook(구 NotebookLM)
노트북 9개로 이동하는 공개 안내 페이지이며, 노트북은 전부 비공개 + 이메일 초대 상태다.
기술자료와 질문하기는 기본적으로 외부망에 공개한다. 중요 자료가 올라오면
`INTRANET_LOCK_ENABLED=true`와 `INTRANET_ALLOWED_IPS`로 사내망에 제한한다. 노트북에 발주처
원본 자료를 올리지 않는다.
