...
- **브랜드 컬러 불일치 (결정 필요).** 요청서 6장 팔레트는 `--color-primary #47b089`(밝은 청록)이나,
  제공된 로고 에셋(`public/logo/README.md`)은 브랜드 녹색을 `#0B6C43`(짙은 녹색)으로 적용했다.
  로고와 UI 액센트 색이 다르면 어색하다. 어느 쪽을 사이트 기준색으로 할지 확정 필요.
  (현재 코드는 요청서 값 `#47b089` 유지 중)
...
남은 확인:
- **배포 도메인**(2번)
- **브랜드 기준색** — 로고 `#0B6C43` vs 요청서 토큰 `#47b089` (7장 참고)
- 로고 에셋 수령 완료 → `public/logo/`. 헤더/푸터/파비콘 적용은 M1.

## 14. 다음 단계

M0(스캐폴드) + 회사소개 페이지 구현 완료. 이후 순서:
1. M1 프로젝트 기반(Prettier/CI, 화면 명세, 공통 컴포넌트 확장, SEO 헬퍼, 테스트 환경)
2. M2 나머지 페이지(사업분야·문의 폼·개인정보·404) — 사업분야/개인정보 콘텐츠 수령 후
3. 배포 도메인 확정 → Resend 연동
